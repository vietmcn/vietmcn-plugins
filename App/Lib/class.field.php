...
class Vietmcn_field
{
    ...
    {
        $checked = ( ! empty( $att['option_checked'] ) ) ? 'checked' : '';
        //
        $out  = '<div class="col-left">';
        $out .= '<label class="cntr tooltip" data-variation="tiny" data-position="left center" data-html="'.$att['desc_popup'].'">';
        $out .= '<span class="lbl"><i class="ion-ios-information-outline"></i>'.$att['option_field_title'].':</span>';
    ...
    {
        $out  = '<div class="ui tooltip col-right" data-variation="tiny" data-position="left center" data-title="Shortcode" data- data-html="'.$att['option_shortcode']['desc'].'">';
        $out .= '<label><i class="ion-gear-a"></i> Shortcode:</label>';
        $out .= '<span class="vietmcn-shortcode">'.$att['option_shortcode']['content'].'</span>';
        $out .= '</div>';
        return $out;
    }
    ...
    {
        $out = '';
        if ( isset( $att['option_field_type']['checkbox'] ) == true ) {

            $out .= self::field_checkbox( $att );

        }
        if ( isset( $att['option_field_type']['shortcode'] ) == true ) {
    ...
    {
        $is_bg = ( isset( $att['option_bg'] ) ) ? $att['option_bg'] : '';
        //
        $out  = '<div class="vietmcn-wrap">';
        $out .= '<div data-elemt="vietmcn-time" class="vietmcn-option-item">';
        $out .= '<div data-elemt="vietmcn-time-thumbnail" style="background:'.$is_bg.'" class="vietmcn-option-item-thumbnail"><i class="'.$att['option_icon'].'"></i></div>';
